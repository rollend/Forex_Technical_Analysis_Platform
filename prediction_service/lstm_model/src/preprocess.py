...
def build_currency_dataframe(buy, sell):
    """
    Builds dataframe containing information for all indicators used in
    lstm training.

    Args:
        buy: String representing symbol of currency being bought
        sell: String representing symbol of currency being sold
        (e.g. build_currency_dataframe("eur", "usd"))
    """
    pair = (buy + sell).upper()
    ...
    pair_df.to_csv(
        "lstm_model/data/processed/{}_processed.csv".format(pair), index=False
    )


def combine_indicators(currency, pair):
    cpi = pd.read_csv(
        "lstm_model/data/interim/cpi/{}_cpi_processed.csv".format(currency)
    )
    gdp = pd.read_csv(
        "lstm_model/data/interim/gdp/{}_gdp_processed.csv".format(currency)
    )
    ir = pd.read_csv(
...
def merge_dataframe(data_list):
    merged_data = reduce(
        lambda left, right: pd.merge(
            left, right, how="outer", on="Time"), data_list
    )
...
def convert_date(exchange):
    exchange["Time"] = pd.to_datetime(
        exchange["Time"], format="%Y-%m-%d %H:%M:%S")
    return exchange
...
if __name__ == "__main__":
    ...
    tweets_df = twitter_sentiment.tweets_merge(
        [forex_com, ft_markets, bloomberg, reuters, wsj, fx_street_1, fx_street_2]
    )
    twitter_sentiment.currency_sentiment(sentiment_keyword, tweets_df)
    print("Twitter sentiment analysis generated...")

    build_currency_dataframe(
        currency_pair[:3].lower(), currency_pair[3:].lower())
